...
app = Flask(__name__, static_folder='temp', static_url_path='/temp',
            template_folder="templates")  # , static_url_path='/temp')
app.config['SESSION_TYPE'] = 'redis'
app.config['SESSION_PERMANENT'] = False
app.config['SESSION_REDIS'] = redis.from_url(
    f"redis://{os.environ['REDIS_IP']}:6379")
    ...
    def wrapper(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if "token" in request.args:
                pushHistoryToELK(request, timespan, usr,
                                 request.args["token"], error)
...
def pushHistoryToELK(request, timespan, usr, token, error):
    ...
    rec = {"url": request.path, "method": request.method, "timespan": timespan,
           "user": usr["login"] if usr else "", "token": token, "error": error, "@timestamp": int(datetime.now().timestamp())*1000}
    if "login" in request.path:
        agent = {
            "browser": request.user_agent.browser, "version": request.user_agent.version, "platform": request.user_agent.platform, "language": request.user_agent.language, "string": request.user_agent.string
        }
        rec["agent"] = agent
    with userlock:
    ...
    def get(self, user=None):
        ...
        if len(files) == 0:
            ...
            if os.path.isfile(objpath):
                ...
                ret = send_file(os.path.abspath(
                    f"./zip_folder/{zip_file_name}"), attachment_filename=files[0], cache_timeout=5)
            ...
            for fil in files:
                ...
                logger.info(
                    f"****{fil}   -> {objpath}    -  {os.path.isfile(objpath)}")
...
def computeMenus(usr, token, apptag):
    ...
    if elkversion == 7:
        res3 = es.search(size=1000, index="nyx_app", body={
                         "sort": [{"order": "asc"}]})
    if elkversion == 8:
        res3 = es.search(size=1000, index="nyx_app", query={
                         "match_all": {}}, sort=[{"order": "asc"}])
    else:
        res3 = es.search(size=1000, index="nyx_app", doc_type="doc", body={
                         "sort": [{"order": "asc"}]})
    ...
    for app in res3["hits"]["hits"]:
        ...
        if "subcategory" in appl and appl["subcategory"] in categories[appl["category"]]["subcategories"]:
            target = categories[appl["category"]
                                ]["subcategories"][appl["subcategory"]]
    ...
    for key in categories:
        ...
        finalcategory.append(
            {"category": key, "loc_category": loc_cat, "submenus": []})
        ...
        for key2 in categories[key]:
            ...
            for key3 in categories[key][key2]:
                ...
                target["submenus"].append(
                    {"title": key3, "loc_title": loc_sub, "apps": []})
                for appli in categories[key][key2][key3]:
                    ...
                    appli["loc_title"] = get_translated_item(
                        language, "menus", appli["title"])
        ...
        if ("login" in data) and ("password" in data):
            ...
            try:
                if elkversion > 6:
                    ...
                    usr = es.get(index="nyx_user",
                                 doc_type="doc", id=cleanlogin)
            except:
                ...
                if elkversion == 8:
                    ...
                    users = es.search(index="nyx_user",
                                      doc_type="doc", body=body)
                # logger.info(users)
            ...
            if usr != None and pbkdf2_sha256.verify(data["password"], usr["_source"]["password"]):
                ...
                if usr["_source"].get("doublePhase", False) == True:
                    if "doublecode" in data:
                        ...
                        redisserver.set(
                            "nyx_double_"+data["login"], randint, 120)
                        logger.info("Code is "+randint)
                        conn.send_message("/topic/AUTH_SMS", json.dumps(
                            {"message": "Your access code is:"+randint, "phone": usr["_source"]["phone"]}))
...
def login_second_step(usr, data):
    ...
    with tokenlock:
        # TO BE DONE REMOVE PREVIOUS TOKENS OF THIS USER
...
class logout(Resource):
    ...
    def get(self, user=None):
        ...
        response.data = json.dumps(
            {"error": "", "azureLogoutUrl": os.environ["AZURE_AUTHORITY"]+"/oauth2/v2.0/logout"})
...
class reset_password(Resource):
    ...
    def post(self, user=None):
        ...
        redisserver.set("nyx_tok_"+str(token),
                        json.dumps(usrdb["_source"]), 60*10)
        conn.send_message("/queue/FORGOTTEN_PASSWORD", json.dumps(
            {"byuser": user, "foruser": usrdb["_source"], "token": token}))
...
class reset_password(Resource):
    ...
    def post(self, user=None):
        ...
        if elkversion > 6:
            res = es.index(index="nyx_user",
                           body=usrdb["_source"], id=req["login"])
        else:
            res = es.index(
                index="nyx_user", body=usrdb["_source"], doc_type="doc", id=req["login"])

        usrdb["_source"]["id"] = usrdb["_id"]

        if "queue" in req:
            conn.send_message(req["queue"], json.dumps(
                {"byuser": user, "foruser": usrdb["_source"], "newpassword": req["new_password"]}))
...
class change_password(Resource):
    ...
    def post(self, user=None):
        ...
        if pbkdf2_sha256.verify(req["old_password"], usrdb["_source"]["password"]):
            ...
            usrdb["_source"]["password"] = pbkdf2_sha256.hash(
                req["new_password"])
            if elkversion > 6:
                res = es.index(index="nyx_user",
                               body=usrdb["_source"], id=user["id"])
            else:
                res = es.index(
                    index="nyx_user", body=usrdb["_source"], doc_type="doc", id=user["id"])
            logger.info(res)
            return {"error": ""}
        else:
...
class azureGetLink(Resource):
    def get(self):
        ...
        if os.environ.get("SKIP_ACTIVE_DIRECTORY", False):
            response.data = json.dumps(
                {"error": "", "skipActiveDirectory": True})
            return response
        else:
            ...
            if session.get("user") != None and session.get("extra_data") != None and logged_out != "true":
                ...
                response.data = json.dumps(
                    {"error": "", "url": "", "signedIn": False, "azureSignedIn": True})
                return response
            if session.get("flow") == None:
                host_url = request.headers["Referer"]
                endpoint = os.environ['AZURE_REDIRECT_ENDPOINT'][1:] if os.environ[
                    'AZURE_REDIRECT_ENDPOINT'][0] == "/" else os.environ['AZURE_REDIRECT_ENDPOINT']
                if os.environ.get("LOCAL") == "true":
                    host_url = ""
                    endpoint = os.environ["AZURE_REDIRECT_ENDPOINT"]
                session["flow"] = _build_auth_code_flow(
                    scopes=["User.Read", "email"], authority=os.environ["AZURE_AUTHORITY"], request_url=f"{host_url}{endpoint}")
            response.data = json.dumps(
                {"error": "", "url": session["flow"]["auth_uri"], "signedIn": False, "azureSignedIn": False})
            return response
...
class azureGetToken(Resource):
    def get(self):
        ...
        me_data = requests.get(msgraph_endpoint, headers={
...
def upload_file(user=None):
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            logger.info(user)
            data = file.read()
            conn.send_message(queue, base64.b64encode(data), {"file": file.filename, "token": request.args.get(
                'token'), "user": json.dumps(user), "upload_headers": request.headers.environ.get('HTTP_UPLOAD_HEADERS')})
            return {"error": ""}
    return {"error": ""}
...
class genericQueryFilter(Resource):
    ...
    def post(self, rec_id, user=None):
        ...
        if "selected" in data:
            ...
            selected = ["" for i in range(
                0, len(app["config"]["queryfilters"])+1)]
        ...
        for index, queryf in enumerate(app["config"]["queryfilters"]):
            if queryf["type"] == "queryselecter":
                ...
                query = {"from": 0, "size": 0, "aggregations": {
                    qcol: {"terms": {"field": qcol, "size": size, "order": [{"_key": "asc"}]}}}}
                query["query"] = cui[1]
                ...
                if index > 0 and len(alladdqueries[index-1]) > 0 and len(addquery) > 0 and "query" in query and "bool" in query["query"] and "must" in query["query"]["bool"]:
                    query["query"]["bool"]["must"][0]["query_string"][
                        "query"] = "("+query["query"]["bool"]["must"][0]["query_string"]["query"]+") AND " + alladdqueries[index-1]
                ...
                # logger.info()
                queryf["buckets"] = res["aggregations"][qcol].get(
                    "buckets", [])
...
class genericSearchPG(Resource):
    ...
    def post(self, appid, user=None):
        ...
        data = json.loads(request.data.decode("utf-8"))
        return loadPGData(es, appid, get_postgres_connection(), conn, data, (request.args.get("download", "0") == "1"), True, user, request.args.get("output", "csv"), OUTPUT_URL, OUTPUT_FOLDER)
...
class genericSearch(Resource):
    ...
    def post(self, index, user=None):
        ...
        return loadData(es, conn, index, data, request.args.get("doc_type", "doc"), (request.args.get("download", "0") == "1"), cui, True, user, request.args.get("output", "csv"), OUTPUT_URL, OUTPUT_FOLDER)
...
class extLoadDataSource(Resource):
    ...
    def get(self, dsid, start=None, end=None, user=None):
        ...
        if querytype == "postgres":
            ...
            r = requests.post(
                'http://esnodebal:9200/_opendistro/_sql', json={"query": query})
...
def pg_genericCRUD(index, col, pkey, user=None):
    ...
    logger.info("PG Generic Table="+index+" Col:" +
                col+" Pkey:" + pkey+" Method:"+met)
    ...
    if met == 'get':
        ...
        if pkey != "NEW":
            ...
            cols = ",".join(["\""+str(_["key"])+"\"='" +
                            str(_["value"])+"' " for _ in data["record"]])
            query += cols
...
def genericCRUD(index, object, user=None):
        ...
        try:
            ...
            if index == "nyx_user":
                ...
                if ("$pbkdf2-sha256" not in dataobj["password"]):
                    dataobj["password"] = pbkdf2_sha256.hash(
                        dataobj["password"])
            ...
            if 'update' in dataobj:
                ...
                if elkversion > 6:
                    ...
                    es.update(index=index, body=data, doc_type=request.args.get(
                        "doc_type", "doc"), id=object)
                ret = logger.info(ret)
            else:
                if elkversion > 6:
                    es.index(index=index, body=data, id=object)
                else:
                    es.index(index=index, body=data, doc_type=request.args.get(
                        "doc_type", "doc"), id=object)
        except:
    ...
    send_event(user=user, indice=index, method=met, _id=object,
                ...
                if (len(apis) > 0):
                    ...
                    for api in apis:
                        ...
                        if elkversion > 6:
                            ...
                            action["index"] = {
                                "_index": indexdatepattern, "_type": "doc"}
...
def refresh_indices():
    ...
    if elkversion == 7:
        ...
        indices = es.search(index="nyx_indice", query={
                            "match_all": {}})["hits"]["hits"]
    else:
        indices = es.search(index="nyx_indice", body={},
                            doc_type="doc")["hits"]["hits"]
...
def refresh_translations():
    ...
    if elkversion == 7:
        translationsrec = es.search(index="nyx_translation", body={
                                    "size": 1000})["hits"]["hits"]
    elif elkversion > 7:
        translationsrec = es.search(index="nyx_translation", query={
                                    "match_all": {}}, size=1000)["hits"]["hits"]
    else:
        translationsrec = es.search(index="nyx_translation", body={
                                    "size": 1000}, doc_type="doc")["hits"]["hits"]
            ...
            if "filtercolumn" in ind["_source"] and ind["_source"]["filtercolumn"] != "":
                if "filters" in user and len(user["filters"]) > 0:
                    ...
                    if len(oldquery) == 0:
                        ...
                        query["bool"]["must"][queryindex]["query_string"]["query"] = oldquery + \
                            " AND ("+newquery+")"
                        return (True, query, resultsmustbefiltered)
...
logger.info(server)
conn = amqstompclient.AMQClient(server, {"name": MODULE, "version": VERSION, "lifesign": "/topic/NYX_MODULE_INFO"}, [
                                '/topic/LOGOUT_EVENT', '/topic/NYX_LAMBDA_RESTAPI'], callback=messageReceived)
...
if os.environ["ELK_SSL"] == "true":
    ...
    es = ES([host_params], http_auth=(os.environ["ELK_LOGIN"],
            os.environ["ELK_PASSWORD"]), verify_certs=True)
else:
