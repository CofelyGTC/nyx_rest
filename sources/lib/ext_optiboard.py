    ...
    class optiboardGetToken(Resource):                    
        def get(self):
            ...
            try:
                ...
                token=uuid.uuid4()
                if record["accepted"]==1:
                    redis.set("nyx_kibana_"+str(token),json.dumps({'return':"OK", 'device': 'optiboard', "login": guid}),3600*24)
                    redis.set("nyx_kibananyx_"+str(token),json.dumps({'return':"OK", 'device': 'optiboard', "login": guid}),3600*24)
